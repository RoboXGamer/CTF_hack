open the lab
the host ip and port we get
then we need to connect using nc -v host port
and then we leak the canary and the PIE address
by using the format string vulnerability of `%9$p %11$p`

then we need to calculate the address of the secret function
by using the leaked canary and the PIE address

for that we use a python script code
...
then we use this python brute force script
and we get the flag
